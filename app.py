...
def load_llm_model():
    """通过 API 动态加载 LLM"""
    # 现在不再加载本地模型，而是通过 API 获取 LLM 响应
    return None, None  # 不需要返回模型和tokenizer


# 预加载默认 LLM（实际上此时不再需要初始化本地模型）
...
async def chat_with_rag(question, chatbot, max_tokens, temperature, top_p, show_debug, topk_retrieval, dist_threshold):
    ...
    elapsed_time = time.time() - start_time
    elapsed_str = f"⏳ Processed in {elapsed_time:.2f} seconds."
...
def create_gradio_interface():
    ...
    with gr.Blocks(theme=theme, title="DeepSeek RAG System 2.0") as interface:
        gr.Markdown("# 🔍 基于LLMs API 的 RAG 知识问答系统 (DeepSeek乞丐版)")

        # 对话区
        chatbot = gr.Chatbot(
            value=[(None, "您好！我是胖橘，您的智能助手 🚀")],
            height=680,
        ...
        with gr.Accordion("🤖 模型管理", open=False):
            model_selector = gr.Dropdown(
                label="选择模型",
                choices=["deepseek-r1-distill-qwen-1.5b", "another-model-name"],  # 列出API支持的模型
                value=config.LLM_MODEL_NAME  # 默认值为当前模型
            )
            model_status = gr.Textbox(label="模型状态", interactive=False, value=f"当前模型: {config.LLM_MODEL_NAME}")
